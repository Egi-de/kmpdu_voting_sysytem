...
export default function Ballot() {
  ...
  const [receiptData] = useState({
    hash: `KMPDU-BLK-${Math.random().toString(36).substring(2, 10).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`,
    token: `KMPDU-VRF-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
  ...
  useEffect(() => {
    ...
    const hasVotedAllNational = nationalPositions.length > 0 ? nationalPositions.every(p => hasUserVotedForPosition(p.id)) : true;
    const hasVotedAllBranch = branchPositions.length > 0 ? branchPositions.every(p => hasUserVotedForPosition(p.id)) : true;
  ...
  }, [allPositions, userVotedPositions, user?.branch, user?.role, hasUserVotedForPosition]);
  ...
  const handleSubmitPhase = async () => {
    ...
    try {
      ...
      // The reactive useEffect will handle the phase transition
    ...
    return (
      <DashboardLayout title="Voting Complete">
        <div className="max-w-xl mx-auto py-8 px-4 animate-fade-in text-center">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold mb-2">Vote Submitted!</h1>
            ...
            <CardContent className="p-4 space-y-4">
               <div className="space-y-3">
                  <div className="flex flex-col gap-1">
                     <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">Position Selected</span>
                     <span className="text-xs sm:text-sm font-semibold truncate">Secretary General</span>
                  </div>
                  
                  <div className="flex flex-col gap-1">
                     <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">Blockchain Hash</span>
                     <div className="flex items-center justify-between gap-2 bg-muted/50 p-2 rounded text-[10px] sm:text-xs font-mono break-all leading-tight">
                        <span className="flex-1 opacity-80">{receiptData.hash}</span>
                        <Button 
                          variant="ghost" 
                          size="icon" 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     <div className="flex flex-col gap-1">
                        <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">Verification Token</span>
                        <span className="text-[10px] sm:text-xs font-mono opacity-80">{receiptData.token}</span>
                     </div>
                     <div className="flex flex-col gap-1">
                        <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">Timestamp</span>
                        <span className="text-[10px] sm:text-xs font-mono opacity-80">{receiptData.timestamp}</span>
                     </div>
                  </div>
               </div>
          ...
          <div className="space-y-4">
             {showThankYou ? (
                <div className="py-4 animate-bounce">
                   <h2 className="text-xl font-bold text-primary">Thank you for voting!</h2>
                   <p className="text-sm text-muted-foreground mt-1">Logging you out securely...</p>
                </div>
             ) : (
                <>
                  <Button 
                  ...
                  <div className="flex flex-col items-center gap-2 pt-4">
                     ...
                     <div className="text-[10px] text-muted-foreground/60 flex items-center gap-2">
                        Auto-logout in <span className="font-bold text-primary w-4">{logoutCountdown}s</span>
                     </div>
                  </div>
                </>
             )}
        ...
        <Card className="border-0 shadow-md">
          <CardHeader className="pb-4 p-4 sm:p-6">
            <div className="flex items-center gap-3 mb-2">
               <Badge variant={phase === 'national' ? 'default' : 'secondary'} className="text-[10px] sm:text-xs">
                   {phase.toUpperCase()}
               </Badge>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <CardTitle className="text-xl sm:text-3xl">{currentPosition.title}</CardTitle>
                ...
                return (
                  ...
                  >
                     <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 overflow-hidden min-w-0">
                          <div className="flex justify-between items-start gap-1">
                             <h3 className="font-semibold text-xs truncate">{candidate.name}</h3>
                             <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 min-w-fit border-emerald-200 text-emerald-700 bg-emerald-50">
                                Active
                             </Badge>
                          </div>
                          <p className="text-[10px] text-muted-foreground line-clamp-1 mt-0.5">{candidate.bio}</p>
                        </div>
                     </div>
                     ...
                     <Button 
                        ...
                        disabled={processingSelection}
                        onClick={() => handleSelectCandidate(candidate.id)}
                        className={`w-full h-8 text-xs ${isSelected ? "bg-green-600 hover:bg-green-700" : ""}`}
                     >
                       {isSelected ? (
                          <>
        ...
        <DialogContent className="w-[90vw] max-w-sm rounded-xl p-4 gap-3">
          ...
          <div className="max-h-[50vh] overflow-y-auto pr-1 space-y-2 py-1">
            {currentPositions.map(position => {
              ...
              return (
                <div key={position.id} className="flex flex-col gap-0.5 py-2 px-3 border border-border/40 bg-muted/20 rounded-lg text-left">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">{position.title}</span>
